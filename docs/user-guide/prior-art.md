...
[pipez](https://github.com/kubuszok/pipez) was an earlier project by Hearth's author that spearheaded the cross-compilation approach used in Chimney 0.8.0 and chimney-macro-commons. It explored how far the abstraction over differences between Scala 2 and Scala 3 macro systems could be pushed, testing which patterns could be shared and where platform-specific code was unavoidable. The lessons from pipez — about what abstractions were practical and where they broke down — fed directly into both Chimney's cross-compilation story and Hearth's design.
